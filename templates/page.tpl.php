...
<div id="wrapper">
  <header id="header" class="clearfix">
    <?php if (theme_get_setting('image_logo','dblog')): ?>
    ...
    <?php endif; ?>
    <?php if (theme_get_setting('socialicon_display', 'dblog')): ?>
        <?php 
        $twitter_url = check_plain(theme_get_setting('twitter_url', 'dblog')); 
        $facebook_url = check_plain(theme_get_setting('facebook_url', 'dblog')); 
        $google_plus_url = check_plain(theme_get_setting('google_plus_url', 'dblog')); 
        $pinterest_url = check_plain(theme_get_setting('pinterest_url', 'dblog'));
        ?>
  ...
  <div id="main" class="clearfix">
    <div id="primary">
      <section id="content" role="main">
        <?php if ($is_front): ?>
        <?php if (theme_get_setting('slideshow_display','dblog')): ?>
        <?php 
        $slide1_url = check_plain(theme_get_setting('slide1_url','dblog'));
        $slide2_url = check_plain(theme_get_setting('slide2_url','dblog'));
        $slide3_url = check_plain(theme_get_setting('slide3_url','dblog'));
        $slide1_desc = check_markup(theme_get_setting('slide1_desc', 'dblog'), 'full_html'); 
        $slide2_desc = check_markup(theme_get_setting('slide2_desc', 'dblog'), 'full_html'); 
        $slide3_desc = check_markup(theme_get_setting('slide3_desc', 'dblog'), 'full_html'); 
        ?>
        <div id="slider">
          <div id="slider-wrap">
            <div class="slides displayblock">
              <a href="<?php print url($slide1_url); ?>"><img width="644" height="320" src="<?php print base_path() . drupal_get_path('theme', 'dblog') . '/images/slides/slide-image-1.jpg'; ?>" class="pngfix"/></a>
              <?php if($slide1_desc) { print '<div class="featured-text">' . $slide1_desc . '</div>'; } ?><!-- .featured-text -->
            </div> <!-- .slides -->

            <div class="slides displaynone">
              <a href="<?php print url($slide2_url); ?>"><img width="644" height="320" src="<?php print base_path() . drupal_get_path('theme', 'dblog') . '/images/slides/slide-image-2.jpg'; ?>" class="pngfix"/></a>
              <?php if($slide1_desc) { print '<div class="featured-text">' . $slide2_desc . '</div>'; } ?><!-- .featured-text -->
            </div> <!-- .slides -->

            <div class="slides displaynone">
              <a href="<?php print url($slide3_url); ?>"><img width="644" height="320" src="<?php print base_path() . drupal_get_path('theme', 'dblog') . '/images/slides/slide-image-3.jpg'; ?>" class="pngfix"/></a>
              <?php if($slide1_desc) { print '<div class="featured-text">' . $slide3_desc . '</div>'; } ?><!-- .featured-text -->
            </div> <!-- .slides -->
          </div>
          <div id="nav-slider">
            <div class="nav-previous"><img class="pngfix" src="<?php print base_path() . drupal_get_path('theme', 'dblog') . '/images/previous.png'; ?>" alt="next slide"></div>
            <div class="nav-next"><img class="pngfix" src="<?php print base_path() . drupal_get_path('theme', 'dblog') . '/images/next.png'; ?>" alt="next slide"></div>
          </div>
        </div>
        <?php endif; ?>
        <?php endif; ?>
  ...
  <footer id="footer-bottom">
    ...
    <div id="bottom" class="clearfix">
      ...
      <div class="credit"><?php print t('by'); ?>  <a href="http://www.geovanirog.wordpress" target="_blank">Geovani</a></div>
    </div>
  </footer>
